...
def get_memory_batch(min_return_to_go):
    ...
    x = torch.stack(
        [memory[possible_ix[i] - block_size + 1:possible_ix[i] + 1] for i in ix])
    x[:, -1, environment.observation_space.shape[0]:] = 0

    y = torch.stack(
        [memory[possible_ix[i] - block_size + 2:possible_ix[i] + 2] for i in
         ix])
...
for episode in range(1, 10000):
    ...
    if episode % 10 == 0:
        ...
        for iter in range(300):
            ...
            xb, yb = get_memory_batch(average_episode_value)
